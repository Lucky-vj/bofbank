...
$data['ex'] = '';
$data['css'] = '';
$data['js'] = '';	//Initialized variables with null or empty
$data['css'] = '.css';
$data['js'] = '.js';		//define js and css extension
...
$data['mdr_fee'] = 0;        // Set Default MDR Fee
$data['mdr_min_fee'] = 0;   // Set Default MDR Min Fee
...
if (isset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] == 'on') $data['Prot'] = 'https';
else $data['Prot'] = 'http';
...
$data['testEmail_1'] = "tech@bofbank.com";
...
if (isset($_SESSION['host_logo']) && $_SESSION['host_logo']) {
	$web_logo = $_SESSION['host_logo'];
} else {
	$web_logo = "images/logo2.png";
}
...
if ($_SESSION['default_IBAN_issuer'] == 45) {
...
function get_iban_list($clientid)
{
	...
	$IBAN_issuer = db_rows("SELECT `IBAN_issuer`,`iban_order_status`,`currency`,`availableBalance` FROM tbl_iban_issuer_account_details WHERE 1 AND client_id='$clientid' AND (Status=1 || iban_order_status=1)");



	foreach ($IBAN_issuer as $key => $list) {
		if ($list['iban_order_status'] == 1) {
			$labelIBAN = "Requested IBAN";
		} else {
			$labelIBAN = get_iban_account_number($clientid, $list['IBAN_issuer']);
		}
		if ($list['IBAN_issuer'] != 3) {
			$list['availableBalance'] = get_user_balance_amt($_SESSION["s_client_id"], $list['IBAN_issuer']);
		}

		echo '<li><a class="dropdown-item changeiban" data-iban="' . $list['IBAN_issuer'] . '" title="Make Default">' . $labelIBAN . ' <span class="float-end ms-2 fw-bold">' . get_currency($list['currency']) . ' ' . $list['availableBalance'] . '</span></a></li>';
	}
}

//Get iban title
function get_global_balance($clientid)
{
	global $data;
	//echo $clientid;
	...
	foreach ($IBAN_issuer as $key => $list) {
		...
		if ($list['IBAN_issuer'] != 3) {
			$getbal = get_user_balance_amt($clientid, $list['IBAN_issuer']);
			$convertamt = currencyConverter($list['currency'], "EUR", $getbal);
			$totamt = $totamt + $convertamt;
		}
	}
	return $totamt;
}
...
//Get Member Details(
function get_member_fees($mid)
{
	global $data;
	$query = "SELECT * FROM `tbl_fee` WHERE `client_id` = '$mid'";
	$result = db_rows($query);
	$fee_details = $result[0];
	return $fee_details;
}
...
function check_username($username)
{
	...
	$query = db_rows("SELECT `client_id` FROM `tbl_client_master` WHERE `username`='$username' AND `banned`=0", 0);
...
function get_iban_account_number($client_id, $IBAN_issuer)
{
	...
	if ($seluserdetail[0]['iban_order_status'] == 1) {
		...
		if (empty($result)) {
			$result = $seluserdetail[0]['accountid'];
		}
		if (empty($result)) {
			$result = $result = "Your IBAN";
		}
	}
	return $result;
}
...
function get_kyc_status($appid)
{
	global $data;
	$kstatus = db_rows("SELECT `Kyc_status` FROM `tbl_client_kyc` WHERE `applicantId`='$appid'");
	$result = $kstatus[0]['Kyc_status'];
	return $result;
}

//Get get iban default currency
function get_iban_currency($ibanid)
{
	global $data;
	$rs = db_rows("SELECT `IBAN_currency`,`IBAN_kyc_provider` FROM `tbl_iban_issuer` WHERE `ID`='$ibanid'", 0);
	$result = $rs[0];
	return $result;
}
...
function generate_account_number($mid, $ibanissuer)
{
	global $data;
	...
	if (isset($format) && $format) {
		$acno = substr($format, 0, 4) . substr("0000000{$mid}", -5);
	} else {
		$acno = date("ym") . substr("0000000{$mid}", -5);
	}
	return $acno;
}
...
{
	global $data;
	$upddetail = db_query("UPDATE `tbl_client_master` SET $fieldnames  WHERE client_id='$memid' AND banned=0");
	return $upddetail;
}
...
function verify_opt($mid, $otp)
{
	...
	$query = "SELECT client_id FROM $tbl_name WHERE otp='{$otp}' AND client_id='{$mid}' AND banned=0";
	$result = db_rows($query);
	$cnt = $data['db_rows_count'];
	return $cnt;
}
...
function generate_otp($mid, $name, $email)
{
	global $data;
	$rand = rand('9087', '2439');
	$fire_Result = db_query("update tbl_client_master set otp='$rand' where client_id='$mid' AND banned=0");
	//////Mail Start/////// 
	$template = "OTP";
...
// fetch Default user balance
function get_user_balance_amt($memid, $ibanid)
{
	global $data;
	$balamt = 0.00;
	$sql = db_rows("SELECT `converted_transaction_amount`,`transaction_type` From `tbl_master_trans_table` WHERE `member_id`='$memid' AND `transaction_status`='Success' AND `iban_id`='$ibanid' ORDER BY `transaction_id` ASC");
	foreach ($sql as $key => $rs) {
...
{
	global $data;
	if (($_SESSION['default_IBAN_issuer'] == 2) && $pid = 1) {
		$pid = 2;
	}
	if (($_SESSION['default_IBAN_issuer'] == 4) && $pid = 1) {
		$pid = 4;
	}
	if (($_SESSION['default_IBAN_issuer'] == 5) && $pid = 1) {
		$pid = 5;
	}
	if (($_SESSION['default_IBAN_issuer'] == 6) && $pid = 1) {
		$pid = 6;
	}
	if (($_SESSION['default_IBAN_issuer'] == 7) && $pid = 1) {
		$pid = 7;
	}
	if (($_SESSION['default_IBAN_issuer'] == 8) && $pid = 1) {
		$pid = 8;
	}
	if (($_SESSION['default_IBAN_issuer'] == 9) && $pid = 1) {
		$pid = 9;
	}
	if (($_SESSION['default_IBAN_issuer'] == 10) && $pid = 1) {
		$pid = 10;
	}
	...
	if ($tableId > 0) {
		...
		$log_slc = db_rows(
			"SELECT * FROM `{$data['DbPrefix']}{$tableName}`" .
				" WHERE `{$tablefieldidname}`='{$tableId}' LIMIT 1",
			$qp
		);
...
?>
			<div class="row json_log_view1_row">
				<div class="col_2" style="padding:0;float:left;">
					<div class="hhh" style="width:0;overflow:hidden;"><? $all_log2 = jsondecode($log); //decode json to php array object
																		?></div>
					...
					<div class="tbl_exl tbl_exlHeightAuto" style="width:98%;overflow:auto">
						<table class="compare" style="margin-top: -2px;">
							<tbody>
								...
								?>
										<tr>
											<td title="<?= $key6; ?>" style="width: 200px;">
												...
												<div style="clear:both;"><?= prndatelog($value6['log_date']); ?>&nbsp;<i class="<?= $data['fwicon']['circle-info']; ?> text-danger" title="<?= $value6['log_action']; ?>"></i></div>
											</td>
	...
	try {
		...
		if ($dbf) {
			...
			if (($db_timestamp < $current_date_2h) || ($exchange_rate_size == 0)) {
				$response_api_get = file_get_contents('https://v6.exchangerate-api.com/v6/2d02663f30bbc1fb7ccfb913/latest/' . $from_Currency);
			...
			$response_json = file_get_contents('https://v6.exchangerate-api.com/v6/2d02663f30bbc1fb7ccfb913/latest/' . $from_Currency);
		}
		...
		if (false !== $response_json) {
			if ($qprint) {
				echo '<br/><br/>' . $response_json . '<br/><br/>';
			}
			$response_object = json_decode($response_json, true);
			if ($qprint) {
				echo '<br/><br/>response_object=>';
			...
			if ('success' === $response_object['result']) {
				...
				if ($qprint) {
					echo '<br/><br/>rates_get=>' . $rates . '';
				}
				if (($dbf) && $to_Currency == $response_object['base_code']) {
					$rates = (1 / (float)$response_object['conversion_rates'][$from_Currency]);
					if ($qprint) {
						echo "<br/>rates {$to_Currency}=>" . $rates . '<br/><br/>';
					}
				}

				$per_rates = number_format((float)(1 * $rates), $result['digit_after_dot'], '.', '');
		...
		$currency_f = number_format((float)$currency_f, '2', '.', '');
...
// fetch transfer reason

function get_transfer_reason($reason_id)
{
	global $data;
	$reason = db_rows("SELECT reason FROM tbl_trasnfer_reason WHERE id='$reason_id' ");
	return $reason[0]['reason'];
}


function get_admin_bank($bank_id)
{
	global $data;
	$bank_fetched = db_rows("SELECT bank_name, account_beneficiary FROM tbl_common_bank_account WHERE bank_id='$bank_id'");
	return $bank_fetched[0];
}

function get_used_bank($tid)
{
	global $data;
	$usedbank = db_rows("SELECT used_bank FROM tbl_master_trans_table WHERE transaction_id='$tid'");
	$result = $usedbank[0]['used_bank'];
	return $result;
}
